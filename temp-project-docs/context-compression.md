...
llmService.streamPrompt(
    ...
    onTokenUpdate = { tokenInfo ->
        ...
        if (usage > 60) {
            println("⚠️ Token 使用率: ${String.format("%.1f", usage)}%")
        }
    },
    onShouldCompress = { current, max ->
        // 压缩提示回调
        println("🔔 建议压缩: 当前 $current / 最大 $max tokens")
    }
).collect { text ->
    print(text) // 实时输出
}
