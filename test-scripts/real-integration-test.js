...
SimpleConfigManager.CONFIG_FILE = path.join(os.homedir(), '.autodev', 'config.yaml');
// 编译后的 mpp-core 模块路径
...
async function checkMppCoreCompilation() {
    console.log('\n🔨 检查 mpp-core 编译状态...');
    try {
...
function createToolRegistry(mppCore) {
    console.log('\n🔧 创建工具注册表...');
    try {
        ...
        const hasWriteFile = tools.some((tool) => tool.name === 'write-file');
...
async function generatePromptWithRealComponents(mppCore, toolRegistry) {
    console.log('\n📝 使用真实组件生成提示词...');
    try {
        ...
        const toolList = tools.map((tool) => `<tool name="${tool.name}">\n  <description>${tool.description}</description>\n  <example>${tool.example}</example>\n</tool>`).join('\n\n');
        // 创建上下文
...
async function callModelWithRealService(mppCore, config, prompt) {
...
async function executeWriteFileFromResponse(toolRegistry, response) {
